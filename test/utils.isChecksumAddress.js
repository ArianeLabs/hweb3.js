    ...
    describe('checkAddressChecksum', function () {
        tests.forEach(function (test) {
            it('shoud test if address ' + test.address + ' passes checksum: ' + test.checksum + ' result: ' + test.is, function () {
                assert.equal(
                    checkAddressChecksum(test.checksum, test.address, clients[test.client]),
                    test.is
                );
            });
        });
        it('should throw error, incorrect address', function () {
            assert.throws(function () { checkAddressChecksum('', '', clients.testnet); }, Error);
            assert.throws(function () { checkAddressChecksum('', '1.0.', clients.testnet); }, Error);
            assert.throws(function () { checkAddressChecksum('', '1.0.1231233', clients.testnet); }, Error);
        });

        it('should throw error, incorrect client', function () {
            assert.throws(function () { checkAddressChecksum('', '0.0.12312412', {}); }, Error);
            assert.throws(function () { checkAddressChecksum('', '0.0.12312412', { _network: { ledgerId: {} } }); }, Error);
            assert.throws(function () { checkAddressChecksum('', '0.0.12312412', { _network: { ledgerId: { _ledgerId: 0 } } }); }, Error);
        });
    });
});
